<template>
  <div>
    <h2>Tareas pendientes {{ totalCompletados }} de un total de {{ totalTareas }}</h2>
    <button style="background-color: red" @click="borrarCompletadas()">Borrar Tareas completadas</button><hr>
    <div class="inputs">
      <input type="text" name="" v-model="valor" v-on:keyup.enter="anadirNota()" v-on:keyup="teclapulsada()" placeholder="Introduce nota" class="input"/>
      <input type="text" name="" value=""  v-model="empiezapor" placeholder="Busca la nota que quieras" class="input2" />
    </div>
    <div v-for="(nota, index) in listaNotasFiltrada" class="principal" v-bind:key="nota.id">
      <div class="dvLinea">{{ nota.Comentario }}
        <div class="">
      ...
      <div class="dvLinea2">{{nota.Fecha}}</div>
    </div>
  </div>
</template>
